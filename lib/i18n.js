...
const de = {
  ...
  // maxOnePerReservation items (e.g. "Hund"/dog) — see lib/dogBooking.js /
  // lib/guest.js's placeGuestOrder. Shown inline once one is already
  // booked (component/guest/GuestApp.jsx), and reused as the booking-time
  // re-check error if the guest still somehow submits one (quantity != 1,
  // or one was already booked by the time the fresh, locked re-check ran).
  alreadyBookedBadge: "Bereits gebucht",
  maxOneDogExplanation: "Pro Apartment ist maximal ein Hund möglich.",
  ...
  // lib/guest.js). Rendered as a single compact card for both reservation
  // phases (see components/guest/GuestApp.jsx's StayExtensionCompactCard);
  // only the subtitle and the collapsed CTA's styling differ by phase.
  stayExtensionTitle: "Eine Nacht länger bleiben",
  // IN-HOUSE phase subtitle — more immediate, since the guest is already
  // staying and could extend right now.
  stayExtensionSubtitle: "Du möchtest noch nicht abreisen? Verlängere Deinen Aufenthalt ganz unkompliziert um eine Nacht.",
  // PRE-ARRIVAL (before_arrival) phase subtitle — softer, planning-oriented,
  // since the stay hasn't started yet.
  ...
  // Only the compact card's EXPANDED confirmation button (either phase)
  // uses this — the collapsed CTA in both phases keeps stayExtensionButton
  // above unchanged.
  stayExtensionFinalButton: "JETZT VERLÄNGERN",
...
const en = {
  ...
  alreadyBookedBadge: "Already booked",
  maxOneDogExplanation: "Maximum one dog per apartment.",
  dogLimitExceededError: "A dog is already booked for this apartment. A maximum of one dog is allowed per apartment.",
  ...
  stayExtensionSubtitle: "Not ready to leave yet? Extend your stay by one more night.",
  stayExtensionCompactSubtitle: "Want to stay another night? Secure your extension night now for less.",
  stayExtensionNewDepartureLabel: "New departure",
  stayExtensionDiscountSuffix: "% less",
  stayExtensionAccommodationLabel: "Extension night",
  ...
  stayExtensionFinalButton: "EXTEND NOW",
