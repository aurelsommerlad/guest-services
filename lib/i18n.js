...
const de = {
  ...
  // "REGULÄRER PREIS" — the undiscounted average nightly rate, shown as a
  // secondary reference next to the prominent total price.
  stayExtensionAverageRateLabel: "REGULÄRER PREIS",
  // "DEIN VERLÄNGERUNGSPREIS" — labels the prominent price (see
  // GuestApp.jsx: this is offer.totalPrice when extras/city tax are also
  // shown, offer.extensionPrice otherwise) in its existing position.
  stayExtensionPriceLabel: "DEIN VERLÄNGERUNGSPREIS",
  // Concatenated directly after the discount percentage (see
  // components/guest/GuestApp.jsx): "20" + " % günstiger" -> "20 % günstiger".
  // Only ever labels the accommodation night specifically — attached right
  // next to that line item whenever extras/city tax are also shown, so it
  // can never be misread as applying to the whole total.
...
const en = {
  ...
  stayExtensionAverageRateLabel: "REGULAR PRICE",
  stayExtensionPriceLabel: "YOUR EXTENSION PRICE",
