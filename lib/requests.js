...
import { hasReachedDogLimit } from "./dogBooking";
import {
  getCatalog,
  getRequestById,
  createRequestRecord,
...
export async function approveRequest(requestId) {
  ...
  try {
    ...
    let reservation, offer, bookingRule, requiredDates, serviceDate, catalogItem;
    try {
      ...
      catalogItem = catalogItems.find((i) => i.serviceId === request.serviceId);
      if (!catalogItem || !catalogItem.active) {
        return { success: false, error: "Dieses Extra ist im Katalog nicht mehr aktiv." };
      }

      // Same max-1-per-reservation rule as the instant path (see
      // lib/dogBooking.js / lib/guest.js's placeGuestOrder) — the request's
      // own requestedQuantity is re-validated here too, since a request can
      // sit pending for a while and this app's own admin-approval flow is
      // itself a second Apaleo booking path that must never bypass the
      // rule. The actual "is one already booked" check happens later,
      // freshly re-verified under a lock immediately before bookService.
      if (catalogItem.maxOnePerReservation && request.requestedQuantity !== 1) {
        return {
          success: false,
          error: "Für dieses Apartment ist bereits ein Hund gebucht. Pro Apartment ist maximal ein Hund möglich.",
        };
      }
    ...
    // maxOnePerReservation items (e.g. "Hund"/dog): claim exclusive
    // booking rights for this reservation+service before the fresh
    // re-check below — this app's own instant-booking path
    // (lib/guest.js's placeGuestOrder) and admin-approval path here both
    // book through the same Apaleo service, so both must serialize on the
    // same lock (see lib/store.js's claimDogBooking) to prevent, e.g., an
    // admin approving two different pending dog requests for the same
    // reservation at the same instant.
    const dogClaimed = catalogItem.maxOnePerReservation
      ? await claimDogBooking(reservation.id, request.serviceId)
      : true;
    if (!dogClaimed) {
      return { success: false, error: "Diese Buchung wird bereits verarbeitet." };
    }
    try {
      if (catalogItem.maxOnePerReservation) {
        // The authoritative, race-condition-safe check — never trust
        // anything computed before this lock was acquired. See
        // lib/dogBooking.js: reads Apaleo's real per-date `count`, never
        // `dates.length`.
        const freshServices = await getReservationServices(reservation.id);
        if (hasReachedDogLimit(freshServices, request.serviceId)) {
          return {
            success: false,
            error: "Für dieses Apartment ist bereits ein Hund gebucht. Pro Apartment ist maximal ein Hund möglich.",
          };
      ...
      if (catalogItem.maxOnePerReservation) await releaseDogBooking(reservation.id, request.serviceId);
    }
