...
// Exposed only for diagnostics (e.g. describing the real storage shape in
// an admin-facing response) — never used to derive a per-reservation key,
// since records are keyed by their own random id, not by reservationId.
export function getExtensionRecordsStorageKey() {
  return extensionRecordsHashKey();
}
