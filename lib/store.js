...
export async function createUser({ username, password, role }) {
  // Imported lazily, only here, rather than as a top-level import: lib/auth.js
  // pulls in next/headers and next/server, which only resolve inside Next's
  // own runtime. A top-level import would force that resolution on every
  // consumer of this module (lib/guest.js included) even when nothing else
  // in this file ever touches auth — e.g. under plain `node --test`.
...
//
// Never booked as an Apaleo service — extends the reservation's actual
// departure date via AmendReservation (see lib/stayExtension.js /
// lib/guest.js). Property-level config, not a fake catalog/service entry,
// so the offer rules can be tuned per property without a code change.

// Discount now depends on reservation phase (before_arrival vs in_house —
// see lib/stayExtension.js's buildExtensionOffer / lib/guest.js's
// determineStayExtensionPhase usage), so the flat
// extensionDiscountOneNightGap/extensionDiscountStandard fields from before
// this change are kept ONLY as a read-side migration fallback (see
// getExtensionConfig below) — production properties already have these
// stored in Redis and must keep working unchanged until they explicitly
// save the new phase-specific fields. Nothing writes the legacy fields
// anymore (see saveExtensionConfig).
...
function extensionConfigKey(propertyId) {
  return `extension-night:config:${propertyId}`;
}
...
/**
 * Reads a property's stay-extension config, resolving each phase-specific
 * discount field with a three-way fallback: the new field itself, then the
 * pre-migration flat field (whatever a property already has stored in
 * Redis), then this module's hard-coded default — in that order. This is
 * what lets existing production config (which only ever had the flat
 * fields) keep producing its existing discount for BOTH phases immediately
 * after deploy, with no migration step, until an admin explicitly saves the
 * new phase-specific fields via the admin panel (see saveExtensionConfig).
 */
export async function getExtensionConfig(propertyId) {
  const stored = (await getJSON(extensionConfigKey(propertyId), null)) || {};
  const legacyOneNightGap = toFiniteNumber(stored.extensionDiscountOneNightGap);
  const legacyStandard = toFiniteNumber(stored.extensionDiscountStandard);

  return {
    extensionNightEnabled: Boolean(stored.extensionNightEnabled ?? DEFAULT_EXTENSION_CONFIG.extensionNightEnabled),
    minSellableStayNights: toFiniteNumber(stored.minSellableStayNights) ?? DEFAULT_EXTENSION_CONFIG.minSellableStayNights,
    extensionDiscountOneNightGap: legacyOneNightGap ?? DEFAULT_EXTENSION_CONFIG.extensionDiscountOneNightGap,
    extensionDiscountStandard: legacyStandard ?? DEFAULT_EXTENSION_CONFIG.extensionDiscountStandard,
    extensionDiscountPreArrivalOneNightGap:
      toFiniteNumber(stored.extensionDiscountPreArrivalOneNightGap) ??
      legacyOneNightGap ??
      DEFAULT_EXTENSION_CONFIG.extensionDiscountPreArrivalOneNightGap,
    extensionDiscountPreArrivalStandard:
      toFiniteNumber(stored.extensionDiscountPreArrivalStandard) ??
      legacyStandard ??
      DEFAULT_EXTENSION_CONFIG.extensionDiscountPreArrivalStandard,
    extensionDiscountInHouseOneNightGap:
      toFiniteNumber(stored.extensionDiscountInHouseOneNightGap) ??
      legacyOneNightGap ??
      DEFAULT_EXTENSION_CONFIG.extensionDiscountInHouseOneNightGap,
    extensionDiscountInHouseStandard:
      toFiniteNumber(stored.extensionDiscountInHouseStandard) ??
      legacyStandard ??
      DEFAULT_EXTENSION_CONFIG.extensionDiscountInHouseStandard,
  };
}

export async function saveExtensionConfig(propertyId, config) {
  const next = {
    extensionNightEnabled: Boolean(config?.extensionNightEnabled),
    extensionDiscountPreArrivalOneNightGap: Number(config?.extensionDiscountPreArrivalOneNightGap),
    extensionDiscountPreArrivalStandard: Number(config?.extensionDiscountPreArrivalStandard),
    extensionDiscountInHouseOneNightGap: Number(config?.extensionDiscountInHouseOneNightGap),
    extensionDiscountInHouseStandard: Number(config?.extensionDiscountInHouseStandard),
    minSellableStayNights: Number(config?.minSellableStayNights),
  };
  await setJSON(extensionConfigKey(propertyId), next);
  return next;
}

function extensionClaimKey(reservationId) {
  return `stay-extension:claim:${reservationId}`;
}

/**
 * Claims exclusive rights to extend a reservation's stay, same
 * atomic-across-instances primitive and purpose as claimOccupancyAmendment
 * above — prevents two concurrent submissions (double-click, two tabs) from
 * both re-checking availability and both amending the reservation. Held
 * only for the duration of a single amendment attempt; callers must
 * releaseStayExtension() in a `finally` block. This is only the
 * concurrency guard — confirmStayExtension additionally re-fetches the
 * reservation and compares its current departure against what the offer
 * was built for, so an already-completed extension (from an earlier,
 * already-finished request) is also refused, not just a simultaneous one.
 */
export async function claimStayExtension(reservationId) {
  return setIfNotExists(extensionClaimKey(reservationId), String(Date.now()));
}

export async function releaseStayExtension(reservationId) {
  await deleteKey(extensionClaimKey(reservationId));
}

// Audit trail only (admin visibility) — never read back to gate booking
// logic, so it deliberately carries no guest-identifying data beyond the
// reservationId already needed to look the booking up in Apaleo.
function extensionRecordsHashKey() {
  return "stay-extensions:records";
}

export async function addExtensionRecord(record) {
  await hashSetField(extensionRecordsHashKey(), record.id, record);
  return record;
}

export async function listExtensionRecords() {
  const map = await hashGetAll(extensionRecordsHashKey());
  return Object.values(map);
}
...
// --- Max-one-per-reservation bookings (maxOnePerReservation, e.g. "Hund"/dog) --

function dogBookingClaimKey(reservationId, serviceId) {
  return `dog-booking:claim:${reservationId}:${serviceId}`;
}

/**
 * Claims exclusive rights to book a maxOnePerReservation catalog item
 * (e.g. a dog service) for a reservation — same atomic-across-instances
 * primitive and purpose as claimVehicleBooking above: prevents two
 * concurrent submissions (double-click, two tabs, a retried request) from
 * both re-checking the existing dog count and both booking. Held only for
 * the duration of a single booking attempt; callers must
 * releaseDogBooking() in a `finally` block. The actual "is a dog already
 * booked" check happens INSIDE the claim (see lib/guest.js's
 * placeGuestOrder), against a freshly re-fetched reservation — the lock
 * only prevents two checks from both seeing "0 booked" at the same time,
 * it is never a substitute for that fresh re-check.
 */
export async function claimDogBooking(reservationId, serviceId) {
  return setIfNotExists(dogBookingClaimKey(reservationId, serviceId), String(Date.now()));
}

export async function releaseDogBooking(reservationId, serviceId) {
  await deleteKey(dogBookingClaimKey(reservationId, serviceId));
}
