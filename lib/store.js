...
export function extensionConfigKey(propertyId) {
...
export async function getExtensionConfig(propertyId) {
  ...
  return { ...DEFAULT_EXTENSION_CONFIG, ...(stored || {}) };
}

/**
 * Returns the exact value stored under the extension-config key, with no
 * defaults merged in (null when nothing has ever been saved for this
 * property). Diagnostic-only — read by the temporary debug endpoints to
 * distinguish "never saved" from "saved but merged with defaults" when
 * comparing environments; not used by any eligibility/booking logic.
 */
export async function getRawExtensionConfig(propertyId) {
  return getJSON(extensionConfigKey(propertyId), null);
}

export async function saveExtensionConfig(propertyId, config) {
  const next = {
    extensionNightEnabled: Boolean(config?.extensionNightEnabled),
    extensionDiscountOneNightGap: Number(config?.extensionDiscountOneNightGap),
    extensionDiscountStandard: Number(config?.extensionDiscountStandard),
