...
/**
 * The total additional amount actually charged by a completed stay
 * extension — the (already discounted) extension night, plus every
 * genuinely NEW per-night extra actually extended (never one flagged
 * `alreadyDone`, i.e. already billed before this extension — that charge
 * isn't additional), plus a verified city-tax charge for the new night
 * when applicable. Shaped around lib/guest.js's confirmStayExtension
 * result objects (`extrasResult`/`cityTax`), NOT the differently-shaped
 * preview objects buildExtensionPricePreview above takes.
 *
 * Returns null whenever the data needed to compute a COMPLETE total isn't
 * available (an extended extra missing its amount, or city tax applicable
 * but never verified) rather than silently under-counting — used both to
 * persist `totalAdditionalAmount` on new audit records at write time
 * (always complete, since every input was just computed) and, identically,
 * to reconstruct the same total for older records that never stored it
 * (see lib/stayExtensionAdmin.js — there it commonly IS incomplete, e.g. a
 * legacy record with no extras/cityTax data at all resolves to just the
 * extension price, which is correct, but one with an extra whose amount
 * was never persisted correctly returns null rather than a wrong number).
 */
export function computeExtensionAdditionalTotal({ extensionPrice, extras, cityTax }) {
  const baseAmount = Number(extensionPrice?.amount);
  if (!Number.isFinite(baseAmount)) return null;
  const currency = extensionPrice.currency || "EUR";
  let amount = baseAmount;

  for (const extra of Array.isArray(extras) ? extras : []) {
    if (!extra?.extended || extra?.alreadyDone) continue;
    const extraAmount = Number(extra?.amount?.amount);
    if (!Number.isFinite(extraAmount)) return null;
    amount += extraAmount;
  }

  if (cityTax?.applicable) {
    const cityTaxAmount = Number(cityTax?.amount?.amount);
    if (!cityTax.verified || !Number.isFinite(cityTaxAmount)) return null;
