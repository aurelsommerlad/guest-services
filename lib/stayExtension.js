...
/**
 * Which visual variant the guest-facing card should render as — purely a
 * presentation signal for the frontend (see components/guest/GuestApp.jsx's
 * StayExtensionCard vs StayExtensionCompactCard), NEVER used to decide
 * whether the offer itself is eligible (see decideExtensionOffer above for
 * that — this runs only once an offer already exists).
 *
 * Driven by Apaleo's own reservation status — the authoritative signal
 * that check-in has actually happened — rather than comparing dates in the
 * browser, which could disagree with the property's actual local
 * arrival/check-in time or the guest's device clock/timezone:
 *   "in_house"        — status is "InHouse": the full, prominent card.
 *   "before_arrival"   — anything else (Confirmed, or any other status an
 *                         offer could in principle still be returned for):
 *                         the compact, discreet card.
 */
export function determineStayExtensionPhase(reservationStatus) {
  return reservationStatus === "InHouse" ? "in_house" : "before_arrival";
}
