...
/**
 * Atomically sets `key` to `value` only if it does not already exist.
 * Returns true if this call created the key, false if it already existed.
 * Used for claim/dedup locks that must be safe across concurrent
 * serverless invocations (e.g. Vercel), where an in-memory lock would not
 * be shared between instances — in production this is a single Redis
 * `SET key value NX` command; locally it is a check-then-set serialized
 * through the same withLocalDbLock queue used by every other local write.
 */
export async function setIfNotExists(key, value) {
  if (hasKv) {
    const kv = await getKvClient();
