...
const APALEO_SCOPES =
  "reservations.read reservations.manage offers.read setup.read folios.read availability.read";
...
export class ApaleoApiError extends Error {
  constructor(message, status, body, { method, endpoint } = {}) {
    ...
    this.status = status;
    this.body = body;
    // Diagnostic-only metadata (never secrets — no header/token/credential
    // ever ends up here) identifying which Apaleo request failed. Additive:
    // every existing catch site only ever reads .status, never these.
...
/**
 * All services currently booked on a reservation, each with its own
 * `dates[]` (one entry per booked service date, carrying that date's own
 * `count`, `amount`, and `isMandatory` flag). Used by the stay-extension
 * flow (lib/stayExtension.js's findExtendableServices) to detect which
 * existing per-night extras need to be extended to a new night, and to
 * verify mandatory/rate-plan-managed services (e.g. final cleaning) are
 * left untouched. Verified live via the Apaleo MCP tool (GetReservationServices)
 * against real HUESLE/ALPILA reservations: a non-mandatory per-night extra
 * like the dog fee has one `dates[]` entry per night of the stay, each with
 * its own already-charged `amount`; a mandatory rate-plan service like
 * "Final cleaning on departure" has exactly one entry, `isMandatory: true`,
 * dated at the reservation's current departure.
 */
export async function getReservationServices(reservationId) {
  if (!reservationId) return [];
  const data = await apaleoFetch(
    `/booking/v1/reservations/${encodeURIComponent(reservationId)}/services`
  );
  return data?.services || [];
}

/**
 * Every folio for a reservation, with charges expanded — used by the
 * stay-extension flow to read the already-posted `CityTax` charge for a
 * specific night. We never derive city tax ourselves from the property's
 * `ListCityTaxes` config: a live comparison found the config's per-person
 * value did not match the actual folio amount (an unexplained surcharge),
 * so Apaleo's own already-computed folio charge is the only value ever
 * trusted for city tax, both for the pre-confirmation price preview (read
 * the last existing night's amount) and for verifying a new night's charge
 * actually appeared after the amendment.
 */
export async function listReservationFolios(reservationId) {
  if (!reservationId) return [];
  const data = await apaleoFetch("/finance/v1/folios", {
    searchParams: { reservationIds: reservationId, expand: "charges" },
  });
  return data?.folios || [];
}
...
/**
 * A service's static definition (availability.mode, postNextDay, ...) —
 * distinct from getServiceLocalized above, which only extracts the
 * localized name/description. Used by the stay-extension flow to confirm
 * a single-date, departure-dated service is genuinely a "moves with
 * checkout" entitlement (availability.mode === "Departure" && postNextDay
 * === true, e.g. Late Check-out) before ever moving it — never inferred
 * from the service's id/code. Verified live via the Apaleo MCP tool
 * against HUESLE-LCO: `{ availability: { mode: "Departure" }, postNextDay:
 * true }`, the only service definition with postNextDay true. Never
 * throws — a lookup failure just means the candidate is treated as not
 * confirmed (left untouched) rather than failing the whole extension.
 */
export async function getServiceDefinition(serviceId) {
  if (!serviceId) return null;
  try {
    return await apaleoFetch(`/rateplan/v1/services/${encodeURIComponent(serviceId)}`);
  } catch (err) {
    console.error(`getServiceDefinition: konnte Service ${serviceId} nicht laden:`, err);
    return null;
  }
}
...
/**
 * Same book-service action as bookService above, and the same replace-not-
 * merge semantics (one PUT must carry the COMPLETE desired date set — see
 * bookService's Fix #4 note), but for a caller that has already computed
 * the full per-date list itself, each entry carrying its OWN `amount`.
 * Needed by the stay-extension flow (lib/stayExtension.js's
 * buildServiceExtensionDates): every existing date must be resent with
 * whatever amount was already charged for it, completely unchanged, while
 * only the newly appended date gets the service's normal (undiscounted)
 * price — bookService's single shared `amount` parameter can't express
 * "same service, different price per date" in one call. `count` at the
 * top level mirrors the highest per-date count in the set, matching how
 * bookService always sends a top-level count alongside per-date entries.
 */
export async function bookServiceDates({ reservationId, serviceId, dates }) {
  const count = Math.max(1, ...dates.map((d) => Number(d.count) || 1));

  console.log(
    `[Apaleo] book-service (stay-extension, per-date amounts) reservationId=${reservationId} serviceId=${serviceId} dates=${dates.length}`
  );
