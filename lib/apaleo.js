import { namesMatch, resolveExternalReferenceMatches } from "./reservationMatching";
 ...
 * childrenAges/timeSlices) — used to increase the guest count for
 * "Extra person"/"Zusatzperson" catalog items (actionType
 * "increase_occupancy"), instead of booking a service. Verified live
 * against a disposable test reservation (see the investigation for this
 * feature): passing an explicit `totalGrossAmount` per time slice with
 * `requote: false` preserves the original per-night price exactly and adds
 * only what's explicitly specified — Apaleo does not silently reprice on
 * top of it. City tax (a PerPersonPerNight charge already tied to the
 * reservation) recalculates automatically from the new `adults` value; no
 * separate AddCityTax call is needed or allowed once city tax already
 * exists on a reservation.
 */
