...
export async function getGuestCatalog(reservation, propertyId) {
  ...
  if (curated.some((item) => item.requiresRemainingCapacity)) {
  ...
  for (const curatedItem of curated) {
    ...
    // requiresRemainingCapacity extras (e.g. "Extra person") are hidden
    // entirely — not shown-but-disabled — once the booked unit group has
    // no remaining guest capacity. See section 5 of the spec: unlike
    // unitGroupRestricted, there is no "shown but blocked" state here.
  ...
  const items = candidates.map((c, i) => {
    ...
    return {
      ...
      // Only set for requiresRemainingCapacity items (null otherwise, i.e.
      // "no cap" — every other item's quantity selector is unaffected).
      // Freshly recomputed here too, so placeGuestOrder's re-check below
      // never trusts a stale value from the guest's browser.
      maxQuantity: curatedItem.requiresRemainingCapacity ? remainingCapacity : null,
    };
  });
