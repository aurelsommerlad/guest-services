 ...
 */
function resolveRequiredDates(bookingRule, offer, reservation) {
...
export async function getGuestCatalog(reservation, propertyId) {
  ...
  const rawCatalog = await getCatalog(propertyId);
  // TEMPORARY DIAGNOSTIC — remove once the LAEKE-PKW "missing from guest
  // catalog" investigation is resolved. Shows exactly what's stored for
  // this property (serviceId/active/bookingRule) before any filtering.
  console.log(
    `[DIAG] getGuestCatalog propertyId=${propertyId} reservation=${reservation?.id}: Redis-Katalog (${rawCatalog.length} Items): ${
      rawCatalog
        .map((i) => `${JSON.stringify(i.serviceId)}(active=${i.active}, rule=${i.bookingRule || "per_stay"})`)
        .join(", ") || "(leer)"
    }`
  );
  const curated = rawCatalog.filter((item) => item.active);
  if (!curated.length) {
  ...
  for (const curatedItem of curated) {
    ...
    if (!offer) {
      ...
      console.log(
        `[DIAG] getGuestCatalog: kein Live-Angebot für kuratiertes serviceId=${JSON.stringify(
          curatedItem.serviceId
        )}. Von Apaleo angebotene serviceIds: ${[...offersByServiceId.keys()].map((id) => JSON.stringify(id)).join(", ")}`
      );
      continue; // not sellable for this reservation right now
    }
    ...
    if (!complete) {
      // TEMPORARY DIAGNOSTIC — see note above.
      console.log(
        `[DIAG] getGuestCatalog: serviceId=${JSON.stringify(curatedItem.serviceId)} rule=${JSON.stringify(
          bookingRule
        )} unvollständig. Von Apaleo für diesen Service verfügbare dates: ${(offer.dates || [])
          .map((d) => d.serviceDate)
          .join(", ")}`
      );
      continue;
