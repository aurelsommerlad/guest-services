...
import { countBookedServiceQuantity, hasReachedDogLimit } from "./dogBooking.js";
...
// Thrown by placeGuestOrder's fresh, immediately-before-booking re-check
// (see lib/dogBooking.js) for a maxOnePerReservation item (e.g. "Hund"/
// dog) — either the requested quantity wasn't exactly 1, or Apaleo's live
// reservation already has one booked, from ANY source (this app, Apaleo
// directly, an OTA, Make). Carries a stable `reason` marker so the API
// route can map it to a specific guest-facing message rather than the
// generic bookingFailedError.
class DogLimitExceededError extends Error {
  constructor() {
    super("Für dieses Apartment ist bereits ein Hund gebucht. Pro Apartment ist maximal ein Hund möglich.");
    this.reason = "dog_limit_exceeded";
  }
}
...
export async function getGuestCatalog(reservation, propertyId) {
  ...
  // Same lazy-fetch principle as remainingCapacity above — only fetched
  // when at least one curated item is flagged maxOnePerReservation (e.g.
  // "Hund"/dog). Counts existing bookings PER SERVICE (see
  // lib/dogBooking.js: never confuses a multi-night dog's several service
  // DATES with multiple dogs), from ANY source (this app, Apaleo directly,
  // an OTA, Make) — never just this app's own cart/order history.
  const dogLimitServiceIds = curated.filter((item) => item.maxOnePerReservation).map((item) => item.serviceId);
  let bookedServiceQuantities = new Map();
  if (dogLimitServiceIds.length) {
    const existingServices = await getReservationServices(reservation.id);
    bookedServiceQuantities = new Map(
      dogLimitServiceIds.map((serviceId) => [serviceId, countBookedServiceQuantity(existingServices, serviceId)])
    );
  }
  ...
  const items = candidates.map((c, i) => {
    ...
    return {
      ...
      // actionType at save time. For a maxOnePerReservation item (e.g.
      // "Hund"/dog), caps at 1 unless one is already booked (then 0 — see
      // alreadyBooked below), so the guest-facing stepper can never even
      // reach quantity 2 in the first place; null otherwise, i.e. "no cap".
      maxQuantity: curatedItem.requiresRemainingCapacity || actionType === "increase_occupancy"
        ? remainingCapacity
        : curatedItem.maxOnePerReservation
        ? (bookedServiceQuantities.get(curatedItem.serviceId) || 0) >= 1
          ? 0
          : 1
        : null,
      // See lib/dogBooking.js — only true for catalog items explicitly
      // configured with maxOnePerReservation (e.g. "Hund"/dog).
      maxOnePerReservation: Boolean(curatedItem.maxOnePerReservation),
      // True once Apaleo's LIVE reservation already has this service
      // booked, from ANY source — this app, Apaleo directly, an OTA, Make.
      // The frontend shows this item as already booked/unavailable instead
      // of an active quantity stepper (see components/guest/GuestApp.jsx);
      // placeGuestOrder re-verifies this fresh, under a lock, immediately
      // before ever calling bookService — this value is presentation only.
      alreadyBooked: curatedItem.maxOnePerReservation
        ? (bookedServiceQuantities.get(curatedItem.serviceId) || 0) >= 1
        : false,
...
export async function placeGuestOrder({ reservation, propertyId, lines, guestName }) {
  ...
  for (const line of lines) {
    ...
    if (item.maxOnePerReservation) {
      // The requested quantity for a max-one-per-reservation item (e.g.
      // "Hund"/dog) must be exactly 1 — 0 never reaches here (filtered by
      // the count<=0 check above), and >1 or any other malformed value is
      // rejected here, never silently clamped to 1. The "is one already
      // booked" check itself happens later, freshly re-verified under a
      // lock immediately before bookService (see below) — this is only
      // the quantity-shape validation.
      if (count !== 1) {
        failed.push({ serviceId: line.serviceId, reason: "dog_limit_exceeded" });
        continue;
      }
    } else if (Number.isFinite(item.maxQuantity) && count > item.maxQuantity) {
    ...
    try {
      // For a maxOnePerReservation item (e.g. "Hund"/dog), claim exclusive
      // booking rights FIRST, before the fresh re-check below — same
      // double-submission guard as claimVehicleBooking, but critically
      // this is what actually prevents two concurrent requests (double
      // click, two tabs, a retried submission) from both re-checking "0
      // booked" at the same instant and both proceeding to book. See
      // lib/store.js's claimDogBooking.
      const dogClaimed = item.maxOnePerReservation
        ? await claimDogBooking(reservation.id, item.serviceId)
        : true;
      if (!dogClaimed) {
      ...
      try {
        if (item.maxOnePerReservation) {
          // The authoritative, race-condition-safe check — never trust the
          // `item.alreadyBooked` snapshot getGuestCatalog computed before
          // this lock was even acquired. Re-fetches the reservation's LIVE
          // services fresh, from inside the lock, and only proceeds if the
          // count is still 0 (see lib/dogBooking.js: reads Apaleo's real
          // per-date `count`, never `dates.length` — a dog already booked
          // for many nights must never be miscounted as multiple dogs, and
          // must never be allowed a second, separate booking here either).
          const freshServices = await getReservationServices(reservation.id);
          if (hasReachedDogLimit(freshServices, item.serviceId)) {
            throw new DogLimitExceededError();
          }
        }
        ...
        if (item.maxOnePerReservation) await releaseDogBooking(reservation.id, item.serviceId);
      }
    } catch (err) {
      failed.push({ serviceId: line.serviceId, reason: err.reason || err.message });
    }
...
export async function getStayExtensionOffer(reservation) {
  try {
    ...
    // Computed once, server-side, from Apaleo's own reservation
    // status/dates — never a client-supplied value. Both the phase-specific
    // discount selection below (buildExtensionOffer) and the offer's
    // exposed `phase` field come from this single calculation, so they can
    // never disagree (see determineStayExtensionPhase).
    const phase = determineStayExtensionPhase({ status: fresh.status, arrivalDate, departureDate });
    ...
    const offer = buildExtensionOffer({ reservation: fresh, timeSlices: fresh.timeSlices, gap, config, phase });
    if (!offer) return null;

    return await attachExtensionPricePreview(offer, fresh, { arrivalDate, departureDate });
  } catch (err) {
...
async function attachExtensionPricePreview(offer, reservation, { arrivalDate, departureDate }) {
  ...
  // offer.phase (set by buildExtensionOffer, from the caller's single
  // determineStayExtensionPhase computation) passes through unchanged here —
  // never recomputed, so the discount that was actually selected and the
  // phase exposed to the frontend can never drift apart.
...
export async function confirmStayExtension({ reservationId, expectedCurrentDeparture }) {
  ...
  try {
    ...
    // Same server-side, never-client-trusted phase computation as
    // getStayExtensionOffer — the price actually amended here must be
    // selected using the same phase the guest's preview was built from.
    const phase = determineStayExtensionPhase({ status: fresh.status, arrivalDate, departureDate: currentDeparture });
    ...
    // because the accommodation amend itself succeeded. propertyId/phase/
    // gap/totalAdditionalAmount are additive (see lib/stayExtensionAdmin.js
    // for the Admin "Verlängerungsnächte" view that reads this record) —
    // every pre-existing field is kept exactly as before, so older records
    // that predate these four fields remain valid, readable records.
    await addExtensionRecord({
      ...
      cityTax,
      mandatoryServicesIntact,
      totalAdditionalAmount: computeExtensionAdditionalTotal({
        extensionPrice: offer.extensionPrice,
        extras: extrasResult,
        cityTax,
      }),
