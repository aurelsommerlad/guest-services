...
async function attachExtensionPricePreview(offer, reservation, { arrivalDate, departureDate }) {
  ...
  // Presentation only (see determineStayExtensionPhase) — never affects
  // eligibility, pricing, or any of the above. departureDate here is the
  // reservation's CURRENT (pre-extension) departure, exactly what "has the
  // stay actually started and not yet ended" needs to check against.
  const phase = determineStayExtensionPhase({ status: reservation.status, arrivalDate, departureDate });
  return { ...offer, extras, cityTax, totalPrice, phase };
}
